...
# Agrega la carpeta 'backend' al sys.path para que los imports funcionen
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'backend')))

from alembic import context
from sqlalchemy import engine_from_config, pool

# Importa tu engine, URL y Base de tu configuración
from backend.app.db.session import engine, MARIADB_URL, Base

# Importa explícitamente todos tus modelos para que Alembic los detecte
...
# Metadata para que Alembic pueda detectar cambios en los modelos
...
def run_migrations_online():
    ...
    with connectable.connect() as connection:
        context.configure(
            ...
            target_metadata=target_metadata,
            compare_type=True,  # útil para detectar cambios en tipos de columnas
        )
        with context.begin_transaction():
            context.run_migrations()
